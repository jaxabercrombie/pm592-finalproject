...
###
# Based on the univariate analyses and clinical signficance of variables, 
#the preliminary final model begins as...
...
# The first preliminary final model indicates statistical significance with 
#creat. phos., ef.fp, hbp, platelets, and sc. Before the variables that are 
#not significant are removed, I will check to make sure potential confounders 
#or effect modifiers do not change things...
...
pfm1.smoke <- glm(death ~ anaemia + creatinine_phosphokinase + ef.fp + 
                    high_blood_pressure + platelets + sc.fp + serum_sodium + 
                    smoking,family = binomial, data = heartfailure)
summary(pfm1)
summary(pfm1.smoke)
